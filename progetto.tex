...
\begin{enumerate}
	\item La società richiede periodicamente ai soci il rinnovo delle quote associative via email, i soci pagano online oppure alla segreteria.
	\begin{enumerate}
		\item \RequisitiSpecifici
				{Ogni 24 ore uno script automatico ricerca tutti gli iscritti la cui quota è in scadenza e invia a questi una email invitanto a rinnovare la quota in via telematica o in sede}
				{Sistema Automatico, Soci Iscritti}
				{Controllo Quote}
		\item \RequisitiSpecifici
				{Il servizio web della segreteria viene aggiornato in modo tale da evidenziare i soci che devono ancora pagare la quota}
				{Sistema Automatico, Front Desk}
				{Aggiornamento interfaccia web}
		\item \RequisitiSpecifici
				{Al pagamento della quota via online, il servizio web è automaticamente aggiornato per mostrare che il socio ha effettivamente rinnovato la quota}
				{Banca, Sistema Automatico, Soci Iscritti, Front Desk}
				{Pagamento Telematico}
		\item \RequisitiSpecifici
				{Al pagamento della quota in sede, il personale può marcare che il socio ha rinnovato la propria quota.}
	...
	\begin{enumerate}
		\item \RequisitiSpecifici
				{I soci della piscina possono accedere all'area riservata del portale web in cui è possibile prenotare un posto in una corsia per allenamento}
				{Soci Iscritti}
				{Login}
		\item \RequisitiSpecifici
				{Il personale FIN può accedere all'interno della proprio area riservate del portale web in cui è possibile prenotare più corsie per gli allenamenti degli atleti}
				{Personale FIN}
				{Login}
		\item \RequisitiSpecifici
				{La compilazione della richiesta è guidata dal sistema e permessa dopo autenticazione.}
				{Soci Iscritti, Personale FIN}
				{Prenotazione Corsie}
		\item \RequisitiSpecifici
				{I dati della prenotazione, previa verifica di disponibilità, sono automaticamente inseriti nel sistema}
				{Sistema Automatico}
				{Allocazioe Corsie}
		\item \RequisitiSpecifici
				{Viene inviata una email per ricordare l'avvenuta prenotazione}
				{Sistema Automatico, Soci Iscritti, Personale FIN}
				{Conferma Prenotazione}
		\item \RequisitiSpecifici
				{Personale autorizzato della società può consultare l'elenco delle prenotazioni per risolvere possibili equivoci a bordo vasca}
	...
	\begin{enumerate}
		\item \RequisitiSpecifici
				{Il prezzo e la disponibilità della struttura sono mostrati in un interfaccia web}
				{Utente Esterno, Sistema Automatico}
				{Visualizzazione Costi}
		\item \RequisitiSpecifici
				{L'utente compila un questionario guidata dalla piattaforma}
				{Utent Esterno, Sistema Automatico}
				{Richiesta Noleggio Struttura}
		\item \RequisitiSpecifici
				{Il sistema notifica al responsabile del reparto ``Rapporti con Enti Esterni'' l'avvenuta richiesta di prenotazione}
	...
	\begin{enumerate}
		\item \RequisitiSpecifici
				{I soci della società, previa autenticazione, hanno accesso ad uno store online dedicato con prodotti forniti esclusivamente da ACQUALIS}
				{Soci Iscritti}
				{Login}
		\item \RequisitiSpecifici
				{Lo store si interfaccia in via telematica alla piattaforma di ACQUALIS la quale comunica i prodotti da mostrare, i prezzi e le quantità disponibili}
				{Sistema Automatico, Soci Iscritti, ACQUALIS}
				{Visulizzazione Prodotti}
		\item \RequisitiSpecifici
				{All'acquisto di un prodotto il pagamento viene gestito completamente da ACQUALIS}
				{ACQUALIS, Soci Inscritti}
				{Pagamento}
		\item \RequisitiSpecifici
				{Dopo l'acquisito viene richiesto se si preferisce ricevere il prodotto a casa o direttamente in piscina, tale richiesta viene inoltrata ad ACQUALIS}
				{ACQUALIS, Soci Inscritti, Sistema Automatico}
				{Estremi spedizione}
		\item \RequisitiSpecifici
				{Nel caso in cui venga deciso di ricevere il prodotto in piscina, il personale viene avvertito e tiene in consegna il prodotto finchè il socio non lo richiede.}
				{Sistema Automatico, Personale, Soci Inscritti}
				{Ricezione Pagamento In Loco}
	\end{enumerate}
\end{enumerate}
...
\begin{ganttchart}[
	...
	\ganttbar{Pianificazione}{12}{25} \\ 
	\ganttbar{Identificazione e analisi requisiti}{26}{41} \\
	\ganttbar{Definizione Architettura}{42}{54} \\
	\ganttbar{Implementazione}{55}{84} \\
	\ganttbar{Definizione UI - UX}{42}{53} \\
	\ganttbar{Implementazione UI - UX}{54}{84} \\
	\ganttbar{Testing}{85}{106} \\
	\ganttbar{Gestione Possibili Cambiamenti}{107}{127} \\
